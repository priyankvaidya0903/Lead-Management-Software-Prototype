...
// ═══════════════════════════════════════════════════════════════════
async function getWorkspace(subdomain) {
    const rows = await query(`SELECT id, "databaseSchema", subdomain, "displayName", "workspaceCustomApplicationId"
     FROM core.workspace WHERE subdomain = $1 LIMIT 1`, [subdomain]);
    if (rows.length === 0)
        throw new Error(`Workspace not found: ${subdomain}`);
    return rows[0];
}
async function getCustomObjects(workspaceId) {
    return query(`SELECT id, "nameSingular", "namePlural", "labelSingular", "labelPlural",
            icon, description
     FROM core."objectMetadata"
     WHERE "workspaceId" = $1 AND "isCustom" = true AND "isActive" = true
     ORDER BY "createdAt"`, [workspaceId]);
}
/** Get non-RELATION custom fields only — relations are handled separately via SQL */
async function getCustomFields(workspaceId, objectId) {
    return query(`SELECT id, name, label, type, description, icon,
     ...
     WHERE "workspaceId" = $1
       ...
       AND "isActive" = true
       AND type != 'RELATION'
     ORDER BY "createdAt"`, [workspaceId, objectId]);
}
...
async function getCustomRelationPairs(workspaceId, customObjectIds) {
    ...
    const allRelFields = await query(`SELECT f.id, f.name, f.label, f.icon, f.type,
     ...
     WHERE f."workspaceId" = $1
       ...
       AND f."isCustom" = true
       AND f."isActive" = true
     ORDER BY f."createdAt"`, [workspaceId]);
    ...
    for (const f of allRelFields) {
        if (seen.has(f.id))
            continue;
        if (autoRelNames.includes(f.name))
            continue;
        // At least one side must be a custom object we're cloning
        if (!customObjSet.has(f.objectMetadataId) && !customObjSet.has(f.relationTargetObjectMetadataId))
            continue;
...
// ═══════════════════════════════════════════════════════════════════
async function getApiToken(workspaceId) {
    ...
    await run(`INSERT INTO core."apiKey" (id, name, "workspaceId", "expiresAt", "createdAt", "updatedAt")
     VALUES ($1, 'setup-tenant-temp', $2, $3, NOW(), NOW())`, [keyId, workspaceId, expiresAt]);
    return jwt.sign({ sub: workspaceId, type: "API_KEY", workspaceId }, ACCESS_TOKEN_SECRET, { expiresIn: "1h", jwtid: keyId });
}
async function cleanupTempApiKey(workspaceId) {
    if (process.env.REAL_API_TOKEN)
        return;
    await run(`DELETE FROM core."apiKey" WHERE name = 'setup-tenant-temp' AND "workspaceId" = $1`, [workspaceId]);
}
// ═══════════════════════════════════════════════════════════════════
//  Phase 3: GraphQL Metadata API helpers
// ═══════════════════════════════════════════════════════════════════
async function metadataGql(token, gql, variables) {
...
async function discoverMutations(token) {
    ...
    const objectMutation = fields.find((n) => /createOneObject/i.test(n)) || "createOneObjectMetadataItem";
    const fieldMutation = fields.find((n) => /createOneField/i.test(n)) || "createOneFieldMetadataItem";
...
async function discoverFieldInputShape(token) {
    try {
        const data = await metadataGql(token, `{
...
// ═══════════════════════════════════════════════════════════════════
async function createObject(token, mutationName, obj) {
    const objectInput = {
    ...
    try {
        const data = await metadataGql(token, `mutation($input: CreateOneObjectInput!) {
...
async function createField(token, mutationName, targetObjectId, field, allowedFields) {
    ...
    try {
        const data = await metadataGql(token, `mutation($input: CreateOneFieldMetadataInput!) {
...
// ═══════════════════════════════════════════════════════════════════
async function cloneRelationsViaSql(sourceWs, targetWs, objectIdMap, relationPairs) {
    // Build a full ID remap: source objectId → target objectId
    // Also include standard (non-custom) objects by querying the target
    const allSourceObjects = await query(`SELECT id, "nameSingular" FROM core."objectMetadata" WHERE "workspaceId" = $1`, [sourceWs.id]);
    const allTargetObjects = await query(`SELECT id, "nameSingular" FROM core."objectMetadata" WHERE "workspaceId" = $1`, [targetWs.id]);
    // Map source object IDs to target object IDs by nameSingular
    const fullObjectIdMap = new Map();
    for (const [srcId, tgtId] of objectIdMap.entries()) {
        fullObjectIdMap.set(srcId, tgtId);
    }
    for (const srcObj of allSourceObjects) {
        if (fullObjectIdMap.has(srcObj.id))
            continue;
    ...
    const fieldMetaCols = await query(`SELECT column_name, is_generated FROM information_schema.columns
    ...
    const [targetAppRow] = await query(`SELECT DISTINCT "applicationId" FROM core."fieldMetadata"
     WHERE "workspaceId" = $1 AND "applicationId" IS NOT NULL LIMIT 1`, [targetWs.id]);
    ...
    for (const { from, to } of relationPairs) {
        ...
        try {
            ...
            const existing = await query(`SELECT id FROM core."fieldMetadata"
         WHERE "workspaceId" = $1 AND "objectMetadataId" = $2 AND name = $3 AND type = 'RELATION'`, [targetWs.id, mappedFromObjectId, from.name]);
            ...
            const buildRow = (src, newId, objId, targetObjId) => {
                const row = { ...src };
                row.id = newId;
                ...
                if ('universalIdentifier' in row)
                    row.universalIdentifier = crypto.randomUUID();
            ...
            for (const row of [newFrom, newTo]) {
                ...
                await run(`INSERT INTO core."fieldMetadata" (${quotedCols}) VALUES (${placeholders})
           ON CONFLICT (id) DO NOTHING`, vals);
            }
            // PASS 2: Now UPDATE both to set the cross-references
            await run(`UPDATE core."fieldMetadata" SET "relationTargetFieldMetadataId" = $1 WHERE id = $2`, [newToId, newFromId]);
            await run(`UPDATE core."fieldMetadata" SET "relationTargetFieldMetadataId" = $1 WHERE id = $2`, [newFromId, newToId]);
            ...
            for (const [settings, objId] of [[fromSettings, mappedFromObjectId], [toSettings, mappedToObjectId]]) {
...
// ═══════════════════════════════════════════════════════════════════
async function cloneWorkflows(sourceWs, targetWs, objectIdMap) {
    const srcSchema = sourceWs.databaseSchema;
    const tgtSchema = targetWs.databaseSchema;
    // Check if workflow table exists in source
    const tableCheck = await query(`SELECT table_name FROM information_schema.tables
     WHERE table_schema = $1 AND table_name = 'workflow'`, [srcSchema]);
    ...
    const wfCols = (await query(`SELECT column_name, is_generated FROM information_schema.columns
     WHERE table_schema = $1 AND table_name = 'workflow'
     ORDER BY ordinal_position`, [srcSchema])).filter((c) => c.is_generated !== 'ALWAYS' && !excludeCols.has(c.column_name))
        .map((c) => c.column_name);
    const wvCols = (await query(`SELECT column_name, is_generated FROM information_schema.columns
    ...
    for (const wf of workflows) {
        ...
        try {
            ...
            const tgtWfCols = (await query(`SELECT column_name, is_generated FROM information_schema.columns
         WHERE table_schema = $1 AND table_name = 'workflow'`, [tgtSchema])).filter((c) => c.is_generated !== 'ALWAYS' && !excludeCols.has(c.column_name))
                .map((c) => c.column_name);
            ...
            await run(`INSERT INTO "${tgtSchema}".workflow (${quotedCols}) VALUES (${placeholders})
         ON CONFLICT (id) DO NOTHING`, vals);
            console.log("✅");
        }
        catch (e) {
    ...
    for (const wv of workflowVersions) {
        ...
        try {
            const tgtWvCols = (await query(`SELECT column_name, is_generated FROM information_schema.columns
         WHERE table_schema = $1 AND table_name = 'workflowVersion'`, [tgtSchema])).filter((c) => c.is_generated !== 'ALWAYS' && !excludeCols.has(c.column_name))
                .map((c) => c.column_name);
            ...
            const placeholders = safeCols.map((_, i) => `$${i + 1}`).join(", ");
            const quotedCols = safeCols.map((c) => `"${c}"`).join(", ");
            await run(`INSERT INTO "${tgtSchema}"."workflowVersion" (${quotedCols}) VALUES (${placeholders})
         ON CONFLICT (id) DO NOTHING`, vals);
            console.log("✅");
        }
        catch (e) {
    ...
    const triggerTableCheck = await query(`SELECT table_name FROM information_schema.tables
     WHERE table_schema = $1 AND table_name = 'workflowAutomatedTrigger'`, [srcSchema]);
    if (triggerTableCheck.length > 0) {
        ...
        if (triggers.length > 0) {
            ...
            const srcTrigCols = (await query(`SELECT column_name FROM information_schema.columns
         WHERE table_schema = $1 AND table_name = 'workflowAutomatedTrigger'`, [srcSchema])).map((c) => c.column_name);
            const tgtTrigCols = (await query(`SELECT column_name FROM information_schema.columns
         WHERE table_schema = $1 AND table_name = 'workflowAutomatedTrigger'`, [tgtSchema])).map((c) => c.column_name);
            for (const trig of triggers) {
                try {
                    ...
                    await run(`INSERT INTO "${tgtSchema}"."workflowAutomatedTrigger" (${quotedCols}) VALUES (${placeholders})
             ON CONFLICT (id) DO NOTHING`, vals);
                }
                catch { /* skip */ }
            }
...
async function main() {
    ...
    if (!sourceSubdomain || !targetSubdomain) {
        console.error("Usage: npx tsx scripts/setup-tenant-api.ts " +
    ...
    try {
        ...
        const sourceFieldsByObject = {};
        for (const obj of sourceObjects) {
            const fields = await getCustomFields(sourceWs.id, obj.id);
            sourceFieldsByObject[obj.id] = fields;
            console.log(`  ${obj.labelSingular}: ${fields.length} non-relation field(s) — [${fields.map((f) => f.name).join(", ")}]`);
        }
        // Read custom relation pairs
        const customRelPairs = await getCustomRelationPairs(sourceWs.id, sourceObjects.map((o) => o.id));
        ...
        const objectIdMap = new Map(); // sourceId → targetId
        for (const obj of sourceObjects) {
            try {
        ...
        for (const hw of sourceWebhooks) {
            try {
                ...
                await run(`INSERT INTO core.webhook (id, "targetUrl", operations, description, secret, "workspaceId", "createdAt", "updatedAt", "universalIdentifier", "applicationId")
           VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW(), $7, $8)`, [newId, hw.targetUrl, hw.operations, hw.description, hw.secret, targetWs.id, crypto.randomUUID(), hw.applicationId]);
                console.log("✅");
            }
            catch (e) {
                console.log(`❌ ${e.message}`);
        ...
        for (const obj of sourceObjects) {
            ...
            try {
                ...
                if (existing.length === 0) {
                    const newId = crypto.randomUUID();
                    await run(`INSERT INTO core."navigationMenuItem" (id, "workspaceId", "targetObjectMetadataId", name, type, "createdAt", "updatedAt", "universalIdentifier")
             VALUES ($1, $2, $3, $4, 'object', NOW(), NOW(), $5)`, [newId, targetWs.id, targetObjectId, obj.namePlural, crypto.randomUUID()]);
                    console.log("✅");
                }
                else {
                    console.log("✅ (already exists)");
        ...
        await run(`UPDATE core.workspace SET "metadataVersion" = "metadataVersion" + 1 WHERE id = $1`, [targetWs.id]);
