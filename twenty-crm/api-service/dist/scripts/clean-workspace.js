...
async function tableExists(tableName) {
    ...
    const res = await query(`SELECT count(*) FROM information_schema.tables WHERE table_schema = 'core' AND table_name = $1`, [clean]);
...
async function main() {
    ...
    // 1. Resolve workspace
    const workspaces = await query(`SELECT id, "databaseSchema", subdomain, "displayName", "workspaceCustomApplicationId"
    ...
    for (const table of CORE_METADATA_TABLES) {
        ...
        const hasWorkspaceId = await query(`SELECT count(*) FROM information_schema.columns
       WHERE table_schema = 'core' AND table_name = $1 AND column_name = 'workspaceId'`, [clean]);
    ...
    const schemaTables = await query(`SELECT table_name FROM information_schema.tables
     WHERE table_schema = $1 AND table_type = 'BASE TABLE'
     ORDER BY table_name`, [ws.databaseSchema]);
