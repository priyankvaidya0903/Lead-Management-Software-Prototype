...
async function main() {
    if (!TOKEN)
        throw new Error("REAL_API_TOKEN is required");
    ...
    const relMutations = fields.filter((f) => f.name.toLowerCase().includes("relation"));
    console.log("\nFound relation mutations:");
    console.log(JSON.stringify(relMutations, null, 2));
    // If no relation mutations, look for createOneField
    const fieldMutation = fields.find((f) => f.name.toLowerCase().includes("createonefield"));
    if (fieldMutation) {
