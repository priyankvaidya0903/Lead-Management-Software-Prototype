 ...
 */
export async function getManagersForClinic(clinicId) {
    // Return cached result if still fresh
    const cached = managerCache.get(clinicId);
    if (cached && Date.now() - cached.fetchedAt < CACHE_TTL_MS) {
        console.log(`[TwentyCRM] Cache hit for clinic ${clinicId}: ${cached.managers.length} manager(s)`);
    ...
    if (!apiKey) {
        console.warn("[TwentyCRM] TWENTY_API_KEY is not set — cannot fetch managers. " +
    ...
    try {
        ...
        // e.g. { data: { managers: [ {...}, {...} ] } }
        const rawManagers = data?.data?.[objectName] ??
            data?.[objectName] ??
            data?.data ??
            [];
 ...
 */
export function clearManagerCache(clinicId) {
    if (clinicId) {
...
export async function findLeadByPhone(phoneNumber) {
    const apiUrl = process.env.TWENTY_API_URL || "http://localhost:3000/rest";
    const apiKey = process.env.TWENTY_API_KEY;
    if (!apiKey)
        return null;
    try {
        const fetchLead = async (phone) => {
        ...
        if (leads && leads.length > 0)
            return { id: leads[0].id, relationshipManagerId: leads[0].relationshipManagerId };
...
export async function createEscalationTask(leadId, managerId, message) {
    const apiUrl = process.env.TWENTY_API_URL || "http://localhost:3000/rest";
    const apiKey = process.env.TWENTY_API_KEY;
    if (!apiKey)
        return false;
