 ...
 */
export async function getNextManagerIndex(clinicId, totalManagers) {
    if (totalManagers <= 0) {
 ...
 */
export async function getRoundRobinState(clinicId) {
    const redis = getRedisClient();
    if (!redis)
        return null;
 ...
 */
export async function resetRoundRobin(clinicId) {
    const redis = getRedisClient();
    if (!redis)
        return;
...
export async function getNextManagerId(clinicId, managerIds) {
...
export async function getCurrentManagerId(clinicId) {
    const redis = getRedisClient();
    if (!redis)
        return null;
    const value = await redis.get(`rr:clinic:${clinicId}`);
