        ...
        if (clinicId) {
            try {
                ...
                if (managers.length > 0) {
                    ...
                    console.log(`[Leads API] Round-robin assigned manager [${managerIndex + 1}/${managers.length}]:`, assignedManager.name, `for clinic: ${clinicId}`);
                }
                else {
                    console.warn(`[Leads API] No managers found for clinicId: ${clinicId}. ` +
                        "Lead will be created without a manager assignment.");
                }
            }
            catch (err) {
                console.error("[Leads API] Manager round-robin failed (lead will still be captured):", err);
            }
        }
