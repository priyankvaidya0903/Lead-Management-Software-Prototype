 ...
 */
async function resolveIdToName(field, id) {
    if (!id)
        return "(empty)";
    ...
    try {
        let endpoint = "";
        if (field === "clinics_id")
            endpoint = `clinicss/${id}`;
        else if (field === "relationship_manager_id")
            endpoint = `managerss/${id}`;
        else
            return id; // Non-relational fields — return raw value
 ...
 */
async function createChangeLogNote(leadId, changes) {
    const apiUrl = process.env.TWENTY_API_URL || "http://localhost:3000/rest";
    const apiKey = process.env.TWENTY_API_KEY;
    if (!apiKey)
        return false;
    try {
        ...
        const targetRes = await fetch(`${apiUrl}/noteTargets?filter=targetLeadsId[eq]:${leadId}&limit=10`, { headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" } });
        if (targetRes.ok) {
...
router.post("/", async (req, res) => {
    try {
        ...
        const leadId = req.headers["id"] || payload?.id;
        if (!leadId) {
        ...
        for (const field of TRACKED_FIELDS) {
            ...
            const val = req.headers[headerKey] || payload?.[field] || "";
            if (val)
        ...
        for (const field of TRACKED_FIELDS) {
            ...
            if (!newVal)
                continue; // Field not sent in this webhook
            if (newVal === oldVal)
                continue; // No change
 ...
 */
router.get("/seed/:leadId", async (req, res) => {
